    ...
    try:
        ...
        match e:
            case "return":
                return "return;"
            case "if":
                return "if (" + genIntExpr() + ") {" + genVoidStmt() + "}"
            case "while":
                return "while (" + genIntExpr() + ") {" + genVoidStmt() + "}"
            case "repeat":
                return "repeat (" + genIntExpr() + ") {" + genVoidStmt() + "}"
            case "assign":
                return genIntVar() + ":=" + genIntExpr() + ";"
        return ""
    finally:
...
def genStringFun():
    global stringfs
    fresh = genFreshVar()
    ret = "fun " + fresh + " string () {" + genStringStmt() + "}"
    strings.append(fresh)
    return ret
...
for i in range(topLevelFuns):
    match random.choice(["int", "string", "void"]):
        case "int":
            print(genIntFun())
        case "string":
            print(genStringFun())
